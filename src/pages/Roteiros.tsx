...
export default function Roteiros() {
  ...
  const startNavigation = (route: RouteItem) => {
    setActiveNavigation({
      route,
      currentStop: 0,
      visited: new Array(route.stops.length).fill(false),
    });
  ...
  const advanceStop = (markVisited: boolean) => {
    ...
    const newVisited = [...activeNavigation.visited];
    if (markVisited) newVisited[activeNavigation.currentStop] = true;

    const nextStop = activeNavigation.currentStop + 1;
    if (nextStop >= activeNavigation.route.stops.length) {
      // Last stop — show conclusion
      setActiveNavigation({ ...activeNavigation, visited: newVisited });
      setShowConclusion(true);
    } else {
      setActiveNavigation({ ...activeNavigation, currentStop: nextStop, visited: newVisited });
    }
  };
  ...
  if (activeNavigation && showConclusion) {
    const visitedCount = activeNavigation.visited.filter(Boolean).length;
    const skippedCount = activeNavigation.visited.filter(v => !v).length;
    return (
        ...
        <main className="max-w-2xl mx-auto px-4 pb-20 flex flex-col items-center justify-center min-h-[70vh]">
          <div className="text-center space-y-4 animate-scale-in">
            ...
            <p className="text-sm text-muted-foreground max-w-xs mx-auto">
              Você completou o roteiro <span className="font-semibold text-foreground">"{activeNavigation.route.title}"</span>
            </p>
            ...
            <div className="pt-4 space-y-2 w-full max-w-xs mx-auto">
              {activeNavigation.route.stops.map((stop, i) => (
                <div key={i} className="flex items-center gap-3 text-left">
                  <div className={cn(
                    "w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold shrink-0",
                    activeNavigation.visited[i]
                      ? "bg-green-500 text-white"
                      : "bg-secondary text-muted-foreground"
                  )}>
                    {activeNavigation.visited[i] ? <CheckCircle2 className="w-4 h-4" /> : i + 1}
                  </div>
                  <span className={cn(
                    "text-sm",
                    activeNavigation.visited[i] ? "text-foreground" : "text-muted-foreground line-through"
                  )}>{stop.name}</span>
                </div>
              ))}
            </div>
  ...
  if (activeNavigation) {
    const { route, currentStop, visited } = activeNavigation;
